...
| `budget` | budget | stop after command count exceeds configured maximum | **Covered for step count, partial for resource risk.** Under the default review set an exceedance holds the run for a named human (one continuation with a run-scoped ceiling raise); removing the rule from `POLICY_REVIEW_RULES` terminates instead. A single expensive command, token spend, subprocess fan-out, and long wall time are not bounded by this counter. | T4 |
...
| `monitor` | command-policy detects; monitor records without termination; redaction sanitises evidence | observe, not block | **Covered as shadow semantics, not prevention.** A detected attack proceeds by design; undetected attacks produce no observation. Budget remains enforcing (as a hold under the default review set, not a monitor-mode observation). | T1, T2, T3, T4 |
