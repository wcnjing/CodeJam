...
| 0.3 | **DONE** — `scripts/preflight.mjs`, `npm run doctor` | Node/npm versions, container engine, ports 3000/5173/9099, Ark key shape, `ARK_MODEL`, a live `ARK_BASE_URL` probe with the BytePlus-vs-Volcengine 401 hint, and a real 16-token inference call that separates a reachable endpoint from a usable one (a model paused on a spend limit answers `200` on `/models` and `429 SetLimitExceeded` on `/responses`; `--no-inference` skips it). Exits non-zero on hard failure; warnings never block |
